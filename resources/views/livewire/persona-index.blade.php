                ...
                <thead>
                    <tr>
                        <th>
                            ...
                            <a href="{{route('personas.index',['condicional'=>'id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                ID
                                <?php
                        ...
                        <th>
                            <a href="{{route('personas.index',['condicional'=>'name','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Name')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='name') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                        ...
                        <th>
                            <a href="{{route('personas.index',['condicional'=>'email','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Email')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='email') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                        ...
                        <th>
                            <a href="{{route('personas.index',['condicional'=>'departamento_id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Department')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='departamento_id') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                ...
                <tbody>
                    @foreach ($personas as $persona)
                        <tr>
                            ...
                            <td>
                                @can('personas.show')
                                    <a class="btn-success btn" href="{{route('personas.show',$persona)}}">{{__('Show')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('personas.edit')
                                    <a class="btn-primary btn" href="{{route('personas.edit',$persona)}}">{{__('Edit')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('personas.destroy')
                                <form action="{{route('personas.destroy',compact('persona'))}}" method="POST">
                                    @csrf
                                    @method('delete')
