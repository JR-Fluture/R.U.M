        ...
        <div class="card-body">
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>
                            <a href="{{route('pcs.index',['condicional'=>'id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                ID
                                <?php
                        ...
                        <th>
                            <a href="{{route('pcs.index',['condicional'=>'microprocesador','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('MicroProcesador')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='microprocesador') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                        ...
                        <th>
                            <a href="{{route('pcs.index',['condicional'=>'placa_base','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Placa Base')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='placa_base') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                        ...
                        <th>
                            <a href="{{route('pcs.index',['condicional'=>'sistema_operativo','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                ...
                <tbody>
                    @foreach ($pcs as $pc)
                        <tr>
                            ...
                            <td>
                                @can('pcs.show')
                                    <a class="btn-success btn" href="{{route('pcs.show',$pc)}}">{{__('Show')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('pcs.edit')
                                    <a class="btn-primary btn" href="{{route('pcs.edit',$pc)}}">{{__('Edit')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('pcs.destroy')
                                    <form action="{{route('pcs.destroy',compact('pc'))}}" method="POST">
                                        @csrf
                                        @method('delete')
