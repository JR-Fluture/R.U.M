    ...
    <div class="card-header container">
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
            ...
            <div class="col-span-2 grid grid-cols-2 gap-6">
                ...
                <div class="col-span-1 w-full h-full flex flex-col justify-center">
                    @can('usuarios.create')
                        <a class="btn btn-primary" href="{{route('usuarios.create')}}">{{__('Add User')}}</a>
                    @endcan
                </div>
            </div>
        ...
        <div class="card-body">
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>
                            <a href="{{route('usuarios.index',['condicional'=>'id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                ID
                                <?php
                        ...
                        <th>
                            <a href="{{route('usuarios.index',['condicional'=>'name','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Name')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                        ...
                        <th>
                            <a href="{{route('usuarios.index',['condicional'=>'email','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Email')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                ...
                <tbody>
                    @foreach ($usuarios as $usuario)
                        <tr>
                            ...
                            <td>
                                @can('usuarios.edit')
                                    <a class="btn-primary btn" href="{{route('usuarios.edit',$usuario)}}">{{__('Edit')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('usuarios.destroy')
                                    <form action="{{route('usuarios.destroy',compact('usuario'))}}" method="POST">
                                        @csrf
                                        @method('delete')
