        ...
        <div class="card-body">
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>
                            <a href="{{route('impresoras.index',['condicional'=>'id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                ID
                                <?php
                        ...
                        <th>
                            <a href="{{route('impresoras.index',['condicional'=>'marca','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Marca')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                                        if ($_SESSION['condicional']=='marca') {
                                            if (isset($_SESSION["orden"])) {
                                                if ($_SESSION['orden']=='ASC') {
                                                    ?>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"  style="width: 20px; height: 20px;"><path d="M288.662 352H31.338c-17.818 0-26.741-21.543-14.142-34.142l128.662-128.662c7.81-7.81 20.474-7.81 28.284 0l128.662 128.662c12.6 12.599 3.676 34.142-14.142 34.142z"/></svg>
                        ...
                        <th>
                            <a href="{{route('impresoras.index',['condicional'=>'modelo','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('Model')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                        ...
                        <th>
                            <a href="{{route('impresoras.index',['condicional'=>'numero_serie','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('NºSerie')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                ...
                <tbody>
                    @foreach ($impresoras as $impresora)
                        <tr>
                            ...
                            <td>
                                @can('impresoras.show')
                                    <a class="btn-success btn" href="{{route('impresoras.show',$impresora)}}">{{__('Show')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('impresoras.edit')
                                    <a class="btn-primary btn" href="{{route('impresoras.edit',$impresora)}}">{{__('Edit')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('impresoras.destroy')
                                    <form action="{{route('impresoras.destroy',compact('impresora'))}}" method="POST">
                                        @csrf
                                        @method('delete')
