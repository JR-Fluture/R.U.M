        ...
        <div class="card-body">
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>
                            <a href="{{route('usopcs.index',['condicional'=>'id','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                ID
                                <?php
                        ...
                        <th>{{__('Monitor')}}</th>
                        <th>
                            <a href="{{route('usopcs.index',['condicional'=>'en_uso','orden'=>isset($_SESSION["orden"]) ? $_SESSION['orden'] : 'ASC'])}}" class="text-black flex">
                                {{__('In use')}}
                                <?php
                                    if (isset($_SESSION["condicional"])) {
                ...
                <tbody>
                    @foreach ($usopcs as $usopc)
                        <tr>
                            ...
                            <td>
                                @can('usopcs.show')
                                    <a class="btn-success btn" href="{{route('usopcs.show',$usopc)}}">{{__('Show')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('usopcs.edit')
                                    <a class="btn-primary btn" href="{{route('usopcs.edit',$usopc)}}">{{__('Edit')}}</a>    
                                @endcan
                            </td>
                            <td>
                                @can('usopcs.destroy')
                                <form action="{{route('usopcs.destroy',compact('usopc'))}}" method="POST">
                                    @csrf
                                    @method('delete')
