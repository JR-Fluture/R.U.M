        ...
        <div class="container py-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">            
                    ...
                    <div class="container-lg bg-white shadow-md overflow-auto p-3">
                        <div>
                            <div class="card">
                                <div class="card-body">
                                    {!! Form::model($contrato, ['class' => 'form-horizontal', 'autocomplete'=>'off']) !!}
                                        @include('contratos.partials.form')
                                        <br>
                                        <div class="form-group">
                                            <h5><strong>{!! Form::label('created_at', __('Creation date')) !!}</strong></h5>
                                            {!! Form::date('created_at', $contrato->created_at, ['class' => 'form-control']) !!}
                                        </div>
                                        <br>
                                        <div class="form-group">
                                            <h5><strong>{!! Form::label('updated_at', __('Edition date')) !!}</strong></h5>
                                            {!! Form::date('updated_at', $contrato->updated_at, ['class' => 'form-control']) !!}
                                        </div>
                                    {!! Form::close() !!}
                                </div>
                            </div>
                ...
                <aside style="border:1px solid black; border-radius: 25px;" class=" shadow">
                    <div class="m-3">
                        <h1 class="text-2xl font-bold text-grey-600 mb-4 text-center">{{__('Show')}}</h1>
                        
                        <ul style="padding-left: 0px;" class="w-full">
                            <li class="mb-4">
                                <a href="{{route('proveedores.show',$contrato->proveedor)}}">
                                    <div class="card shadow">
                                        <div class="card-body">
                                            <h5 class="card-title"><span class="ml-2 text-gray-600">{{$contrato->proveedor->name}}</span></h5>
                                        </div>
                                    </div>
