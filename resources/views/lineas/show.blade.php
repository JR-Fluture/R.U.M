<x-app-layout>
            ...
            <div class="lg:col-span-2">            
                ...
                <div class="container-lg bg-white shadow-md overflow-auto p-3">
                    <div>
                        <div class="card">
                            <div class="card-body">
                                {!! Form::model($linea, ['class' => 'form-horizontal', 'autocomplete'=>'off']) !!}
                                    @include('lineas.partials.form')
                                    <br>
                                    <div class="form-group">
                                        <h5><strong>{!! Form::label('created_at', __('Creation date')) !!}</strong></h5>
                                        {!! Form::date('created_at', $linea->created_at, ['class' => 'form-control']) !!}
                                    </div>
                                    <br>
                                    <div class="form-group">
                                        <h5><strong>{!! Form::label('updated_at', __('Edition date')) !!}</strong></h5>
                                        {!! Form::date('updated_at', $linea->updated_at, ['class' => 'form-control']) !!}
                                    </div>
                                {!! Form::close() !!}
                            </div>
                        </div>
            ...
            <aside style="border:1px solid black; border-radius: 25px;" class=" shadow">
                <div class="m-3">
                    ...
                    <ul style="padding-left: 0px;" class="w-full">
                        <li class="mb-4">
                            <a href="{{route('contratos.show',$linea->contrato)}}">
                                <div class="card shadow">
                                    <div class="card-body">
                                        <h5 class="card-title"><span class="ml-2 text-gray-600">{{$linea->contrato->IDCONTACTO}}</span></h5>
                                        {{-- <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p> --}}
                                        <p class="card-text ml-2 text-gray-600">  
                                            {{$linea->contrato->proveedor->name}}
                                            <br>
                                            {{__('Description').': '.$linea->contrato->descripcion}}
                                        </p>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $('#tipo_linea_id').attr('disabled','disabled');
    </script>
</x-app-layout>
